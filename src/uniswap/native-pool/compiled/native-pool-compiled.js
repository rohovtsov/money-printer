 ...
 */
var TICK_SPACINGS = (_a = {},
    _a[FeeAmount.LOWEST] = 1,
    _a[FeeAmount.LOW] = 10,
    _a[FeeAmount.MEDIUM] = 60,
    _a[FeeAmount.HIGH] = 200,
    _a);
var MaxUint160 = Math.pow(2n, 160n) - 1n;
...
var POWERS_OF_2 = [128, 64, 32, 16, 8, 4, 2, 1].map(function (pow) { return [
    pow,
    Math.pow(2n, BigInt(pow)),
]; });
var Q32 = Math.pow(2n, 32n);
var Q96 = Math.pow(2n, 96n);
var MAX_FEE = Math.pow(10n, 6n);
...
var TickMath = /** @class */ (function () {
    ...
    TickMath.getSqrtRatioAtTick = function (tick) {
        ...
        // back to Q96
        var result = ratio % Q32 > 0n ? ratio / Q32 + 1n : ratio / Q32;
    ...
    TickMath.getTickAtSqrtRatio = function (sqrtRatioX96) {
        ...
        var sqrtRatioX128 = sqrtRatioX96 << 32n;
        ...
        var log_sqrt10001 = log_2 * BigInt('255738958999603826347141');
        var tickLow = Number((log_sqrt10001 - BigInt('3402992956809132418596140100660247210')) >> 128n);
        var tickHigh = Number((log_sqrt10001 + BigInt('291339464771989622907027621153398088495')) >> 128n);
     ...
     */
    TickMath.MIN_SQRT_RATIO = BigInt('4295128739');
    /**
     * The sqrt ratio corresponding to the maximum tick that could be used on any pool.
     */
    TickMath.MAX_SQRT_RATIO = BigInt('1461446703485210103287273052203988822378723970342');
    TickMath.cache_getSqrtRatioAtTick = [];
    return TickMath;
}());
...
var SqrtPriceMath = /** @class */ (function () {
    ...
    SqrtPriceMath.getNextSqrtPriceFromAmount1RoundingDown = function (sqrtPX96, liquidity, amount, add) {
        if (add) {
            var quotient = amount <= MaxUint160 ? (amount << 96n) / liquidity : (amount * Q96) / liquidity;
...
var SwapMath = /** @class */ (function () {
    ...
    SwapMath.computeSwapStep = function (sqrtRatioCurrentX96, sqrtRatioTargetX96, liquidity, amountRemaining, feePips) {
        var returnValues = {};
        var zeroForOne = sqrtRatioCurrentX96 >= sqrtRatioTargetX96;
        var exactIn = amountRemaining >= 0n;
        if (exactIn) {
            var amountRemainingLessFee = (amountRemaining * (MAX_FEE - BigInt(feePips))) / MAX_FEE;
            ...
            if (amountRemaining * -1n >= returnValues.amountOut) {
                ...
                returnValues.sqrtRatioNextX96 = SqrtPriceMath.getNextSqrtPriceFromOutput(sqrtRatioCurrentX96, liquidity, amountRemaining * -1n, zeroForOne);
            }
        }
...
var NativeTick = /** @class */ (function () {
    function NativeTick(index, liquidityGross, liquidityNet) {
...
var TickList = /** @class */ (function () {
    ...
    TickList.validateList = function (ticks, tickSpacing) {
        ...
        invariant(ticks.every(function (_a) {
            var index = _a.index;
            return index % tickSpacing === 0;
        }), 'TICK_SPACING');
        // ensure tick liquidity deltas sum to 0
        invariant(ticks.reduce(function (accumulator, _a) {
    ...
    TickList.nextInitializedTickWithinOneWord = function (ticks, tick, lte, tickSpacing) {
        ...
        if (lte) {
            ...
            var tickObject = TickList.nextInitializedTick(ticks, tick, lte);
            var nextInitializedTick = Math.max(minimum, tickObject.index);
            ...
            var tickObject = this.nextInitializedTick(ticks, tick, lte);
            var nextInitializedTick = Math.min(maximum, tickObject.index);
            return [nextInitializedTick, nextInitializedTick === tickObject.index ? tickObject : null];
        }
    };
    return TickList;
}());
var NativeTickDataProvider = /** @class */ (function () {
    function NativeTickDataProvider(ticks, tickSpacing) {
        TickList.validateList(ticks, tickSpacing);
        this.ticks = ticks;
    }
    NativeTickDataProvider.prototype.getTick = function (tick) {
        return TickList.getTick(this.ticks, tick);
    };
    NativeTickDataProvider.prototype.nextInitializedTickWithinOneWord = function (tick, lte, tickSpacing) {
        return TickList.nextInitializedTickWithinOneWord(this.ticks, tick, lte, tickSpacing);
    };
    return NativeTickDataProvider;
}());
/**
 * Represents a V3 pool
 */
var NativePool = /** @class */ (function () {
    function NativePool(token0, token1, fee, sqrtRatioX96, liquidity, tickCurrent, ticks) {
        this.token0 = token0;
        this.token1 = token1;
        this.fee = fee;
        ...
        invariant(sqrtRatioX96 >= tickCurrentSqrtRatioX96 && sqrtRatioX96 <= nextTickSqrtRatioX96, 'PRICE_BOUNDS');
     ...
     */
    NativePool.prototype.involvesToken = function (token) {
     ...
     */
    NativePool.prototype.getOutputAmount = function (inputToken, inputAmount, sqrtPriceLimitX96) {
        invariant(this.involvesToken(inputToken), 'TOKEN');
        var zeroForOne = inputToken === this.token0;
        var outputAmount = this.swapFast(zeroForOne, inputAmount);
     ...
     */
    NativePool.prototype.getInputAmount = function (outputToken, outputAmount, sqrtPriceLimitX96) {
        invariant(this.involvesToken(outputToken), 'TOKEN');
        var zeroForOne = outputToken === this.token1;
        var inputAmount = this.swapFast(zeroForOne, outputAmount * -1n);
        //invariant(inputAmount === this.swapFast(zeroForOne, outputAmount * -1n), 'FUCK');
        return inputAmount;
    };
    NativePool.prototype.swap = function (zeroForOne, amountSpecified, sqrtPriceLimitX96) {
        var _a, _b;
        ...
        var exactInput = amountSpecified >= 0n;
        ...
        while (state.amountSpecifiedRemaining !== 0n && state.sqrtPriceX96 !== sqrtPriceLimitX96) {
            ...
            _a = this.tickDataProvider.nextInitializedTickWithinOneWord(state.tick, zeroForOne, this.tickSpacing), step.tickNext = _a[0], step.tickNextInitialized = _a[1];
            step.tickNext = clamp(step.tickNext, TickMath.MIN_TICK, TickMath.MAX_TICK);
            step.sqrtPriceNextX96 = TickMath.getSqrtRatioAtTick(step.tickNext);
            _b = SwapMath.computeSwapStep(state.sqrtPriceX96, (zeroForOne
                ? step.sqrtPriceNextX96 < sqrtPriceLimitX96
                : step.sqrtPriceNextX96 > sqrtPriceLimitX96)
                ? sqrtPriceLimitX96
                : step.sqrtPriceNextX96, state.liquidity, state.amountSpecifiedRemaining, this.fee), state.sqrtPriceX96 = _b[0], step.amountIn = _b[1], step.amountOut = _b[2], step.feeAmount = _b[3];
            /*console.log(
            ...
            if (state.sqrtPriceX96 === step.sqrtPriceNextX96) {
                ...
                if (step.tickNextInitialized) {
                    var liquidityNet = step.tickNextInitialized.liquidityNet;
    ...
    NativePool.prototype.swapFast = function (zeroForOne, amountSpecified) {
        var _a;
        invariant(amountSpecified !== 0n, 'ZERO_AMOUNT_SPECIFIED');
        var sqrtPriceLimitX96 = zeroForOne ? TickMath.MIN_SQRT_RATIO + 1n : TickMath.MAX_SQRT_RATIO - 1n;
        if (zeroForOne) {
            invariant(sqrtPriceLimitX96 < this.sqrtRatioX96, 'RATIO_CURRENT');
        }
        else {
        ...
        var exactInput = amountSpecified >= 0n;
        var stateAmountSpecifiedRemaining = amountSpecified;
        var stateAmountCalculated = 0n;
        var stateSqrtPriceX96 = this.sqrtRatioX96;
        var stateTick = this.tickCurrent;
        var stateLiquidity = this.liquidity;
        // start swap while loop
        while (stateAmountSpecifiedRemaining !== 0n && stateSqrtPriceX96 !== sqrtPriceLimitX96) {
            var stepSqrtPriceStartX96 = void 0;
            var stepTickNext = void 0;
            var stepTickNextInitialized = void 0;
            var stepSqrtPriceNextX96 = void 0;
            var stepAmountIn = void 0;
            var stepAmountOut = void 0;
            var stepFeeAmount = void 0;
            stepSqrtPriceStartX96 = stateSqrtPriceX96;
            // because each iteration of the while loop rounds, we can't optimize this code (relative to the smart contract)
            // by simply traversing to the next available tick, we instead need to exactly replicate
            // tickBitmap.nextInitializedTickWithinOneWord
            _a = this.tickDataProvider.nextInitializedTickWithinOneWord(stateTick, zeroForOne, this.tickSpacing), stepTickNext = _a[0], stepTickNextInitialized = _a[1];
            ...
            var swapStepSqrtPriceNextX96 = (zeroForOne
                ? stepSqrtPriceNextX96 < sqrtPriceLimitX96
                : stepSqrtPriceNextX96 > sqrtPriceLimitX96)
                ? sqrtPriceLimitX96
                : stepSqrtPriceNextX96;
            var swapStepSqrtPriceX96 = stateSqrtPriceX96;
            var swapStepLiquidity = stateLiquidity;
            var swapStepAmountSpecifiedRemaining = stateAmountSpecifiedRemaining;
            if (exactInput) {
                var amountRemainingLessFee = (swapStepAmountSpecifiedRemaining * (MAX_FEE - this.feeBigInt)) / MAX_FEE;
                stepAmountIn = zeroForOne
                    ? SqrtPriceMath.getAmount0Delta(swapStepSqrtPriceNextX96, swapStepSqrtPriceX96, swapStepLiquidity, true)
                    : SqrtPriceMath.getAmount1Delta(swapStepSqrtPriceX96, swapStepSqrtPriceNextX96, swapStepLiquidity, true);
                if (amountRemainingLessFee >= stepAmountIn) {
                    stateSqrtPriceX96 = swapStepSqrtPriceNextX96;
                }
                else {
                    stateSqrtPriceX96 = SqrtPriceMath.getNextSqrtPriceFromInput(swapStepSqrtPriceX96, swapStepLiquidity, amountRemainingLessFee, zeroForOne);
                }
            }
            else {
                stepAmountOut = zeroForOne
                    ? SqrtPriceMath.getAmount1Delta(swapStepSqrtPriceNextX96, swapStepSqrtPriceX96, swapStepLiquidity, false)
                    : SqrtPriceMath.getAmount0Delta(swapStepSqrtPriceX96, swapStepSqrtPriceNextX96, swapStepLiquidity, false);
                if (swapStepAmountSpecifiedRemaining * -1n >= stepAmountOut) {
                    stateSqrtPriceX96 = swapStepSqrtPriceNextX96;
                }
                else {
                    stateSqrtPriceX96 = SqrtPriceMath.getNextSqrtPriceFromOutput(swapStepSqrtPriceX96, swapStepLiquidity, swapStepAmountSpecifiedRemaining * -1n, zeroForOne);
                }
            }
            var max = swapStepSqrtPriceNextX96 === stateSqrtPriceX96;
            if (zeroForOne) {
                stepAmountIn =
                    max && exactInput
                        ? stepAmountIn
                        : SqrtPriceMath.getAmount0Delta(stateSqrtPriceX96, swapStepSqrtPriceX96, swapStepLiquidity, true);
                stepAmountOut =
                    max && !exactInput
                        ? stepAmountOut
                        : SqrtPriceMath.getAmount1Delta(stateSqrtPriceX96, swapStepSqrtPriceX96, swapStepLiquidity, false);
            }
            else {
                stepAmountIn =
                    max && exactInput
                        ? stepAmountIn
                        : SqrtPriceMath.getAmount1Delta(swapStepSqrtPriceX96, stateSqrtPriceX96, swapStepLiquidity, true);
                stepAmountOut =
                    max && !exactInput
                        ? stepAmountOut
                        : SqrtPriceMath.getAmount0Delta(swapStepSqrtPriceX96, stateSqrtPriceX96, swapStepLiquidity, false);
            }
            ...
            if (exactInput && stateSqrtPriceX96 !== swapStepSqrtPriceNextX96) {
                ...
                stepFeeAmount = FullMath.mulDivRoundingUp(stepAmountIn, this.feeBigInt, MAX_FEE - this.feeBigInt);
            }
            ...
            if (stateSqrtPriceX96 === stepSqrtPriceNextX96) {
                ...
                if (stepTickNextInitialized) {
                    var liquidityNet = stepTickNextInitialized.liquidityNet;
                    // if we're moving leftward, we interpret liquidityNet as the opposite sign
                    // safe because liquidityNet cannot be type(int128).min
                    if (zeroForOne) {
                        liquidityNet *= -1n;
    ...
    NativePool.prototype.swapSteps = function (zeroForOne) {
        var _a, _b;
        //TODO: change to while tick <= MAX_TICK / MIN_TICK
        var amountSpecified = 1000000000000000000000000000000000000000000000000000000n;
        var sqrtPriceLimitX96 = zeroForOne
        ...
        while (state.amountSpecifiedRemaining !== 0n && state.sqrtPriceX96 !== sqrtPriceLimitX96) {
            ...
            step.tickNext = clamp(step.tickNext, TickMath.MIN_TICK, TickMath.MAX_TICK);
            step.sqrtPriceNextX96 = TickMath.getSqrtRatioAtTick(step.tickNext);
            var stepSqrtPriceX96 = state.sqrtPriceX96;
            var stepLiquidity = state.liquidity;
            var stepTargetSqrtPriceX96 = (zeroForOne
                ? step.sqrtPriceNextX96 < sqrtPriceLimitX96
                : step.sqrtPriceNextX96 > sqrtPriceLimitX96)
                ? sqrtPriceLimitX96
                : step.sqrtPriceNextX96;
            _b = SwapMath.computeSwapStep(stepSqrtPriceX96, stepTargetSqrtPriceX96, stepLiquidity, state.amountSpecifiedRemaining, this.fee), state.sqrtPriceX96 = _b[0], step.amountIn = _b[1], step.amountOut = _b[2], step.feeAmount = _b[3];
            if (exactInput) {
                state.amountSpecifiedRemaining -= step.amountIn + step.feeAmount;
                state.amountCalculated -= step.amountOut;
            }
...
var NATIVE_POOL = new NativePool('MY_TOKEN0', 'MY_TOKEN1', 500, 3516436670545488550380412n, 983779684705963253n, -200463, [
    new NativeTick(-887270, 0n, 577215028887498n),
    new NativeTick(887270, 0n, -577215028887498n),
]);
