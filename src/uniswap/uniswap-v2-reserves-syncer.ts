...
export class UniswapV2ReservesSyncer {
  ...
  constructor(
    ...
    this.queryContract = new Contract(UNISWAP_LOOKUP_CONTRACT_ADDRESS, UNISWAP_QUERY_ABI, this.provider);
  }
  ...
  async syncReservesBatch(markets: UniswapV2Market[]): Promise<void> {
    ...
    const reserves: Array<Array<BigNumber>> = (await this.queryContract.functions.getReservesByPairs(pairAddresses))[0];
