...
import {
  ...
  LiquidityMath,
  Pool,
  SwapMath, Tick, TICK_SPACINGS, TickConstructorArgs,
  TickList,
...
class AdvancedTickListDataProvider extends TickListDataProvider {
  ...
  nextInitializedTickWithinOneWordSync(tick: number, lte: boolean, tickSpacing: number): [number, boolean] {
...
export class AdvancedPool extends Pool {
  ...
  public getInputAmountSync(
    ...
    const { amountCalculated: inputAmount, sqrtRatioX96, liquidity, tickCurrent } = this.swapSync(
      zeroForOne,
  ...
  private swapSync(
    ...
    while (JSBI.notEqual(state.amountSpecifiedRemaining, ZERO) && state.sqrtPriceX96 != sqrtPriceLimitX96) {
      ...
      ;[step.tickNext, step.initialized] = this.advancedTickDataProvider.nextInitializedTickWithinOneWordSync(
        state.tick,
        zeroForOne,
        this.tickSpacing
      )
      ...
      if (JSBI.equal(state.sqrtPriceX96, step.sqrtPriceNextX96)) {
        ...
        if (step.initialized) {
          let liquidityNet = JSBI.BigInt(this.advancedTickDataProvider.getTickSync(step.tickNext).liquidityNet)
